...
enum class Ecran { Titre, Creation, Reglages, Systeme, Bureau, Contrats, Gestion,
                   Programme, Vol, Carte, VolInterp, Etude, GameOver };
...
struct Interface {
  ...
  void dessiner(float w, float h, double dt_reel) {
    dt_courant = dt_reel;
    // temps reel : la salle de vol vit
    if (ecran == Ecran::Vol || ecran == Ecran::VolInterp) jeu.tick(dt_reel);
    if (ecran == Ecran::Vol || ecran == Ecran::VolInterp) {
      const double sp = (ecran == Ecran::Vol ? 12.0 : 6.0);
      vue.yaw += (float)(dt_reel * 0.06 * ((jeu.vol.tr_en_route || jeu.vinterp.commis) ? 1 : 0.3));
      (void)sp;
    }
    ImGui::SetNextWindowPos(ImVec2(0, 0), ImGuiCond_Always);
    ImGui::SetNextWindowSize(ImVec2(w, h), ImGuiCond_Always);
    ImGui::Begin("##jeu", nullptr,
                 ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove |
                 ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoScrollbar);
    if (jeu.game_over && ecran != Ecran::GameOver && ecran != Ecran::Titre) ecran = Ecran::GameOver;
    const bool barre = jeu.agence.creee && ecran != Ecran::Titre &&
                       ecran != Ecran::Creation && ecran != Ecran::Reglages &&
                       ecran != Ecran::GameOver;
    if (barre) barre_haut();
    switch (ecran) {
      ...
      case Ecran::Systeme:   e_systeme();   break;
      case Ecran::Bureau:    e_bureau();    break;
      case Ecran::Contrats:  e_contrats();  break;
      case Ecran::Gestion:   e_gestion();   break;
      case Ecran::Programme: e_programme(); break;
      case Ecran::Vol:       e_vol();       break;
      case Ecran::Carte:     e_carte();     break;
      case Ecran::VolInterp: e_vol_interp(); break;
      case Ecran::Etude:     e_etude();     break;
    }
    if (barre && jeu.tuto.actif && guide_ouvert) dessiner_guide();
    if (!jeu.erreur.empty()) {
  ...
  void barre_haut() {
    ...
    ImGui::SameLine();
    if (ImGui::SmallButton("Sauver")) jeu.sauvegarder(chemin_sauvegarde);
    ImGui::Separator();
  ...
  // -------------------------------------------------------------------------
  void e_titre() {
    ImGui::Dummy(ImVec2(0, 60)); centre("SPACE PROGRAM", 3.4f);
    ImGui::Dummy(ImVec2(0, 6)); centre("le joueur concoit, le monde propage, la physique tranche");
    ImGui::Dummy(ImVec2(0, 40));
    const float bw = 360, bh = 44;
    ...
    ImGui::Dummy(ImVec2(0, 10)); ImGui::SetCursorPosX((ImGui::GetWindowSize().x - bw) * 0.5f);
    if (ImGui::Button("REPRENDRE (charger)", ImVec2(bw, bh))) {
      if (jeu.charger(chemin_sauvegarde)) ecran = Ecran::Bureau;
      else jeu.erreur = "Aucune sauvegarde lisible.";
    }
    ImGui::Dummy(ImVec2(0, 10)); ImGui::SetCursorPosX((ImGui::GetWindowSize().x - bw) * 0.5f);
    ...
    centre("v0.6 - carte du systeme solaire, missions GEO / Mars / comete / TITAN, tutoriel guide");
    centre("102 oracles physiques au vert - aucun chiffre invente a l'ecran");
  }
  ...
  void e_game_over() {
    ...
    if (ImGui::Button("CHARGER LA SAUVEGARDE", ImVec2(em(19), em(2.6f)))) {
      jeu.reinitialiser();
      if (jeu.charger(chemin_sauvegarde)) ecran = Ecran::Bureau; else ecran = Ecran::Titre;
    }
    ImGui::SetCursorPosX(x);
  ...
  void e_creation() {
    ...
    ImGui::Checkbox("Suivre le tutoriel (une directrice de vol te guide)", &guide_ouvert);
    ImGui::Dummy(ImVec2(0, 10)); ImGui::SetCursorPosX(x);
    if (ImGui::Button("FONDER", ImVec2(520, 42))) {
      jeu.creer_agence(nom_buf, (app::ModeAide)mode_choix);
      jeu.tuto.actif = guide_ouvert; jeu.tuto.etape = 0;
      ecran = Ecran::Bureau;
    }
  }
