...
enum class Ecran { Titre, Creation, Reglages, Systeme, Bureau, Contrats, Gestion,
                   Programme, Vol, Carte, VolInterp, Etude, Ares, Carte3D, GameOver };
...
struct Interface {
  ...
  void dessiner(float w, float h, double dt_reel) {
    ...
    jeu.ares.assurer(jeu.agence, jeu.epoch_courant());
    // pont rendu 3D : le monde UE dessine la carte quand cet ecran est actif
    ...
    // CARTE 3D : fond transparent, le systeme solaire (monde UE) est derriere
    if (ecran == Ecran::Carte3D) flags_jeu |= ImGuiWindowFlags_NoBackground;
    ...
    // retombe sur la carte.
    switch (ecran) {
      case Ecran::Titre: case Ecran::Creation: case Ecran::Reglages:
      case Ecran::GameOver: case Ecran::Carte3D: break;
      default: ecran = Ecran::Carte3D; break;
    }
    if (ecran == Ecran::Carte3D && !jeu.agence.creee) ecran = Ecran::Titre;
    switch (ecran) {
