...
// Les positions publiees sont des ESTIMATIONS de navigation entretenues par
// le sol - jamais une verite absolue [GDD 7.5].
// ---------------------------------------------------------------------------
struct EnginFlotte {
  ...
  // relais / orbiteur : cercle de rayon sma_m (le demi-grand axe REEL atteint
  // par la mission), phase0 = anomalie a t0, sens prograde. Cercle declare.
...
struct Jeu {
  ...
  Tuto tuto;
  // Couche ARES (GDD v1.1) : carriere, arbre 6 branches, catalogue verrouille,
  // Novellus, fiabilite. Synchronisee par l'UI (assurer) ; achats via payer().
  AresLayer ares;
