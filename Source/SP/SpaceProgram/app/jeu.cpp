...
void Jeu::terminer_vol() {
  ...
  if (vol.ok) {
    ...
    if (conception.instrument) {
      ++relais_geo;
      // Mise en service [GDD 8.3] : l'engin garde SON ephemeride, batie sur le
      // demi-grand axe REELLEMENT atteint (el_final), phase tiree de la graine.
      EnginFlotte e; e.type = EnginFlotte::RelaisGeo; e.nom = c.id;
      e.t0 = epoch_courant(); e.sma_m = vol.el_final.a;
      e.phase0 = TWO_PI * (double)(vol.graine % 1000) / 1000.0;
      flotte.push_back(e);
      agence.log("Satellite operationnel : +1 relais science en GEO.");
    }
...
void Jeu::interp_terminer() {
  ...
  if (survol) {
    ...
    if (vinterp.ok) {
      ++sondes_lointaines;
      // Mise en service [GDD 8.3] : apres le survol, la sonde poursuit sur son
      // orbite heliocentrique de sortie : v(corps) + v_inf reel du transfert,
      // direction d'asymptote tiree de la graine du vol (modele de sortie
      // DECLARE, borne par l'energie reelle du survol).
      const State C = etat_cible(vinterp.t_arr);
      const double ang = TWO_PI * (double)(vinterp.graine % 1000) / 1000.0;
      const double vinf = std::max(100.0, cinterp.vinf_arr);
      EnginFlotte e; e.type = EnginFlotte::SondeLointaine; e.nom = c.id;
      e.t0 = vinterp.t_arr; e.r0 = C.r;
      e.v0 = C.v + Vec3{vinf * std::cos(ang), vinf * std::sin(ang), 0.0};
      flotte.push_back(e);
    }
    ...
    if (vinterp.ok) {
      ++orbiteurs_mars;
      // Mise en service [GDD 8.3] : orbite circularisee au periastre REELLEMENT
      // atteint (rp_km), phase tiree de la graine. Cercle ecliptique declare.
      EnginFlotte e; e.type = EnginFlotte::OrbiteurMars; e.nom = c.id;
      e.t0 = vinterp.t_arr; e.sma_m = vinterp.rp_km * 1000.0;
      e.phase0 = TWO_PI * (double)(vinterp.graine % 1000) / 1000.0;
      flotte.push_back(e);
    }
    vinterp.pourquoi = vinterp.ok
